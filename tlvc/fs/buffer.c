...
static void INITPROC add_buffers(int nbufs, char *buf, ramdesc_t seg)
{
	...
	/* FIXME: this may not be the right test for XMS usage */
        offset = nr_xms_bufs? ((n & 63) << BLOCK_SIZE_BITS) :
...
#endif
        {	/* allocate buffers in main memory */
...
#define FORCEMAP 0
    ...
    if (FORCEMAP || bh->b_data || xms_mode > XMS_UNREAL) {	// DANGEROUS!!!
        map_buffer(bh);
