...
int do_mknod(char *pathname, int offst, int mode, dev_t dev)
{
    ...
    debug("DO_MKNOD: dir_namei returned %d\n", error);
    if (!error) {
	dirp = dir;
	if (!namelen) {
...
int sys_link(char *oldname, char *pathname)
{
    ...
    if (!error) {
        ...
        if (error && oldinode->i_count) /* minix_link will fail if oldname is a directory,
					 * then iput the inode before returning. The i_count
					 * check is to silence the freeing free inode message */
            iput(oldinode);
    }
    return error;
#endif
