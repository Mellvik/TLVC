...
#if ANY_UNIX
# if UNIXV
static struct termios	oldtermios;	/* original tty mode */
static struct termios	newtermios;	/* cbreak/noecho tty mode */
# else
...
# endif
	...
	/*refresh();*/	/* removed by grh, overwrites error msg then crashes*/
...
# if UNIXV
		...
		newtermios.c_cc[VINTR] = ctrl('C'); /* always use ^C for interrupts */
